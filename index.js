...
function get20s(array) {
  ...
  for (let i = 0; i < array.length; i++){
    let splitter = array[i].years.split(' ');
    if (splitter[0] >= 1900 && splitter[2] <= 2000){
...
function addArtist(array) {
  array.push('id: 20, name: Your Name Here, years: Your Birth Year - current day, genre: Web Design, nationality: Your Nationality Here, bio: Add 1-2 sentences')
  return array;
}
console.log(artists)
...
function artistByCountry(array, artistNationality){
  let artistFromTheCountry = []
  for (let i = 0; i < array.length; i++){
    if(array[i].nationality === artistNationality){
      artistFromTheCountry.push(array[i].name)
    }
  } return artistFromTheCountry;
}
